...
namespace WebAPINetCore.Services
{
    public class ControladorPerisfericos
    {
        ...
        public bool FloatByDenominationBillete()
        {
            Globals.log.Debug("Inicio del float billete");
            List<string> data = new List<string>();
            data.Add("");
            PipeClient2 FloatBillete = new PipeClient2();
            FloatBillete.Message = Globals.servicio.BuildMessage(ServicioPago.Comandos.FloatBillete, data);
            var respuesta = FloatBillete.SendMessage(ServicioPago.Comandos.FloatBillete);
            if (respuesta)
            {
                return true;
            }
            ...
            {
                Globals.log.Error("Error al obtener nivel  payout Transaccion: + Globals.IDTransaccion +  Error:" + FloatBillete.Resultado.CodigoError + " Respuesta completa " + FloatBillete._Resp);
                return false;
            }
        }
